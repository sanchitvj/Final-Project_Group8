...
def main(cfg):
    ...
    first_ = cfg.model.backbone_name.split('/')[1] if '/' in cfg.model.backbone_name else cfg.model.backbone_name
    wandb.run.name = first_ + '_' + cfg.model.pooling + '_' + str(cfg.training.epochs)
    if cfg.logger.save:
        wandb.run.save()
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_cycles=0.5,
                                                num_warmup_steps=0.1 * len(train) // cfg.dataset.tr_batch_size,
                                                num_training_steps=(
                                                        cfg.training.epochs * len(train) // cfg.dataset.tr_batch_size))
    last_epoch = 0
    ...
    for epoch in range(last_epoch + 1, cfg.training.epochs):
        ...
        if avg_mcrmse < best_score:
            ...
            filepath = os.path.join('saved_checkpoints',
                                    f"{first_}_{cfg.model.pooling}_{epoch}_{best_score:.4f}.pth")
